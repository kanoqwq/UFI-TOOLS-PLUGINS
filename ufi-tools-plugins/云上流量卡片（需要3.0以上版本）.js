...
(()=>{
  ...
  const cacheKey = `ys_traffic_cache_${cardNumber}`;
  const cacheTimeKey = `ys_cache_time_${cardNumber}`;
  ...
  const fetchData = async () => {
    try {
      const res = await fetch(`${KANO_baseURL}/proxy/--http://aaa.ghy6.com/app/simCard/phoneSimCard?card=${cardNumber}`);
