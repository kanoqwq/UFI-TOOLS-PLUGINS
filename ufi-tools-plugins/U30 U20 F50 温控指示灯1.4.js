...
(async () => {
    ...
    const CHECK_TIME = 4; // 检查温度的间隔时间(秒),越小越频繁，越耗电
...
LED_RED="/sys/class/leds/sc27xx:red/brightness"
LED_WHITE="/sys/class/leds/sc27xx:green/brightness"   # 这里其实是白灯
LED_BLUE="/sys/class/leds/sc27xx:blue/brightness"
...
smooth_transition() {
    ...
    for i in $(seq 1 $steps); do
        r=$((cur_r + (tgt_r - cur_r) * i / steps))
        g=$((cur_g + (tgt_g - cur_g) * i / steps))
        b=$((cur_b + (tgt_b - cur_b) * i / steps))
        set_color $r $g $b
        sleep 0.03
    done
}
...
EOF
    ...
    smooth_transition $cur_r $cur_g $cur_b $tgt_r $tgt_g $tgt_b
