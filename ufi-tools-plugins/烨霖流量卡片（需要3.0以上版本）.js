...
(()=>{
  ...
  const cacheKey = `yl_traffic_cache_${cardNumber}`;
  const cacheTimeKey = `yl_cache_time_${cardNumber}`;
  const now = Math.floor(Date.now() / 1000);
  const lastFetch = parseInt(localStorage.getItem(cacheTimeKey) || '0');
  const cachedData = localStorage.getItem(cacheKey);
  const fetchData = async () => {
    try {
      const res = await fetch(`${KANO_baseURL}/proxy/--http://mxkj089.cn/app/simCard/phoneSimCard?card=${cardNumber}`);
